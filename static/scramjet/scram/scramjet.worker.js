(() => {
	...
	"$scramjet" in self ||
		(self.$scramjet = {
			...
			codec: {},
			flagEnabled: (e, r) => {
				const s = t.config.flags[e];
				for (const s in t.config.siteFlags) {
					const i = t.config.siteFlags[s];
					if (new RegExp(s).test(r.href) && e in i) return i[e];
				}
				return s;
			},
		});
	const t = self.$scramjet,
		r = Function,
		{
			util: { BareClient: s, ScramjetHeaders: i, BareMuxConnection: o },
			url: { rewriteUrl: n, unrewriteUrl: a, rewriteBlob: c, unrewriteBlob: l },
			rewrite: {
				rewriteCss: d,
				unrewriteCss: h,
				rewriteHtml: u,
	...
	async function x(e, r) {
		const s = new URLSearchParams(new URL(e.url).search);
		if (s.has("url"))
			return Response.redirect(n(s.get("url"), v(new URL(s.get("url")))));
		try {
			let o = new URL(e.url),
				n = "";
			if (
				...
				s.startsWith("blob:") && (s = l(s));
				const i = await fetch(s, {}),
					c = s.startsWith("blob:") ? s : "(data url)";
				(i.finalURL = c),
					i.body &&
						(t = await R(
							i,
							r
								? {
										base: new URL(new URL(r.url).origin),
										origin: new URL(new URL(r.url).origin),
									}
								: v(new URL(a(e.referrer))),
							e.destination,
							n,
							this.cookieStore,
						));
				const d = Object.fromEntries(i.headers.entries());
				return (
					crossOriginIsolated &&
						((d["Cross-Origin-Opener-Policy"] = "same-origin"),
						(d["Cross-Origin-Embedder-Policy"] = "require-corp")),
					new Response(t, {
						status: i.status,
						statusText: i.statusText,
						headers: d,
			...
			const c = new URL(a(o)),
				d = this.serviceWorkers.find((e) => e.origin === c.origin);
			if (d && d.connected && "swruntime" !== s.get("from")) {
				const t = await d.fetch(e);
				if (t) return t;
			}
			if (c.origin == new URL(e.url).origin)
				throw Error(
					"attempted to fetch from same origin - this means the site has obtained a reference to the real origin, aborting",
				);
			const h = new i();
			for (const [t, r] of e.headers.entries()) h.set(t, r);
			if (r && new URL(r.url).pathname.startsWith(t.config.prefix)) {
				const e = new URL(a(r.url));
				e.toString().includes("youtube.com") ||
					(h.set("Referer", e.toString()),
					h.set("Origin", e.origin ? `${e.protocol}//${e.host}` : "null"));
			}
			const u = this.cookieStore.getCookies(c, !1);
			u.length && h.set("Cookie", u),
				h.set("Sec-Fetch-Dest", e.destination),
				h.set("Sec-Fetch-Site", "same-origin"),
				h.set("Sec-Fetch-Mode", "cors" === e.mode ? e.mode : "same-origin");
			const p = new S(c, e.body, e.method, e.destination, r, h.headers);
			...
			return await k(c, n, e.destination, m, this.cookieStore, r, this);
		} catch (s) {
			const r = {
				message: s.message,
				url: e.url,
			...
			if (
				(s.stack && (r.stack = s.stack),
				console.error("ERROR FROM SERVICE WORKER FETCH: ", r),
	...
	async function k(e, t, r, s, i, o, n) {
		let a;
		const c = g(s.rawHeaders, v(e)),
			l = c["set-cookie"] || [];
		for (const t in l)
			o && o.postMessage({ scramjet$type: "cookie", cookie: t, url: e.href });
		for (const t in (await i.setCookies(l instanceof Array ? l : [l], e), c))
			Array.isArray(c[t]) && (c[t] = c[t][0]);
		if (
			(s.body && (a = await R(s, v(e), r, t, i)),
			["document", "iframe"].includes(r))
		) {
			const e = c["content-disposition"];
		...
		const d = new C(a, c, s.status, s.statusText, r, e, s, o);
		return (
			n.dispatchEvent(d),
	...
	async function R(e, t, r, s, i) {
		switch (r) {
			case "iframe":
			case "document":
				if (e.headers.get("content-type")?.startsWith("text/html"))
					return u(await e.text(), i, t, !0);
				return e.body;
			case "script":
				return f(await e.arrayBuffer(), e.finalURL, t, "module" === s);
			case "style":
				return d(await e.text(), t);
			case "sharedworker":
			case "worker":
				return y(await e.arrayBuffer(), s, e.finalURL, t);
			default:
				return e.body;
		}
	}
	...
	class j extends EventTarget {
		...
		syncPool = {};
		synctoken = 0;
		cookieStore = new t.shared.CookieStore();
		serviceWorkers = [];
		constructor() {
			super(), (this.client = new t.shared.util.BareClient());
			const r = indexedDB.open("$scramjet", 1);
			(r.onsuccess = () => {
				const e = r.result
				...
				addEventListener("message", async ({ data: t }) => {
					if ("scramjet$type" in t) {
						if ("registerServiceWorker" === t.scramjet$type) {
							this.serviceWorkers.push(new e(t.port, t.origin));
							return;
						}
				...
				(e.onsuccess = async () => {
					...
					(o.onsuccess = () => {
						(this.config = o.result),
							(t.config = o.result),
							(t.codec.encode = r("url", t.config.codec.encode)),
							(t.codec.decode = r("url", t.config.codec.decode)),
							s();
					}),
						(o.onerror = () => i(o.error));
				}),
